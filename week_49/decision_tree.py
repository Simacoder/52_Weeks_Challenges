...
with app.setup:
    ...
    def load_california_housing_alternative():
        """Load California housing data from alternative source or use synthetic data"""
        try:
            ...
            print("Successfully loaded real California Housing dataset!")
            return Bunch(data=X.values, target=y.values, 
                        feature_names=X.columns.tolist(),
                        target_names=['MedHouseVal'])
        except Exception as e:
            print(f"Failed to load real data ({e}). Using synthetic data instead...")
            np.random.seed(42)
            n_samples = 20640
            ...
            print("Successfully created synthetic California Housing-like dataset!")
            return Bunch(
                data=synthetic_data,
...
def _(california_housing_df, target):
    ...
    feature_cols = [col for col in california_housing_df.columns if col != target and california_housing_df[col].dtype in ['float64', 'int64']]
    X = california_housing_df[feature_cols]
...
def print_info(dt_suffix):
    ...
    print()
    time_val = globals()[f"time_{dt_suffix}"]
...
def _(dt_shallow, fitpred_loop):
    ...
    return maes_shallow, mapes_shallow, mses_shallow, r2s_shallow, rmses_shallow, time_shallow
...
def _(dt_deep, fitpred_loop):
    ...
    return maes_deep, mapes_deep, mses_deep, r2s_deep, rmses_deep, time_deep
...
def _(dt_prune, fitpred_loop):
    ...
    return maes_prune, mapes_prune, mses_prune, r2s_prune, rmses_prune, time_prune
...
@app.cell
def _(maes_deep, maes_prune, maes_shallow, mapes_deep, mapes_prune, mapes_shallow, mses_deep, mses_prune, mses_shallow, r2s_deep, r2s_prune, r2s_shallow, rmses_deep, rmses_prune, rmses_shallow):
    ...
    _layout = go.Layout(title='Comparison of Metrics for Decision Tree Variants', boxmode='group', xaxis=dict(tickangle=90))
...
def _(maes_deep, maes_prune, maes_shallow):
    ...
    _layout = go.Layout(title='Normalised Comparison of MAEs for Decision Tree Variants', yaxis=dict(title='Mean Absolute Error (Normalised, Mean = 0)'), boxmode='group', width=400, height=400)
    go.Figure(data=_data, layout=_layout)
    return
...
def _(maes_deep, maes_prune, maes_shallow):
    ...
    _data = [_trace_shallow, _trace_deep, _trace_prune]
    _layout = go.Layout(barmode='overlay', title='Overlapping Histogram of MAE Distributions', xaxis=dict(title='Mean Absolute Error'), yaxis=dict(title='Count'), width=800)
    go.Figure(data=_data, layout=_layout)
    return
...
def _(fitpred_loop, fitpred_single):
    ...
    print_info("split")
    return maes_split, mapes_split, mses_split, r2s_split, rmses_split, time_split
...
def _(fitpred_loop, fitpred_single):
    ...
    print_info("leaf")
    return maes_leaf, mapes_leaf, mses_leaf, r2s_leaf, rmses_leaf, time_leaf
...
def _(fitpred_loop, fitpred_single):
    ...
    print_info("max")
    return dt_max, maes_max, mapes_max, mses_max, r2s_max, rmses_max, time_max
...
def _(dt_max):
    ...
    _total_samples = _tree.n_node_samples[0]
...
def _(dt_bscv, fitpred_loop):
    ...
    return maes_bscv, mapes_bscv, mses_bscv, r2s_bscv, rmses_bscv, time_bscv
...
@app.cell
def _(maes_bscv, maes_deep, maes_prune, maes_shallow, mapes_bscv, mapes_deep, mapes_prune, mapes_shallow, mses_bscv, mses_deep, mses_prune, mses_shallow, r2s_bscv, r2s_deep, r2s_prune, r2s_shallow, rmses_bscv, rmses_deep, rmses_prune, rmses_shallow):
    _trace_maes_shallow = go.Box(y=maes_shallow, name='MAEs, Shallow Tree', marker_color='blue', width=0.6)
    _trace_maes_deep = go.Box(y=maes_deep, name='MAEs, Deep Tree', marker_color='green', width=0.6)
    _trace_maes_prune = go.Box(y=maes_prune, name='MAEs, Prune Tree', marker_color='red', width=0.6)
    ...
    _data = [_trace_maes_shallow, _trace_maes_deep, _trace_maes_prune, _trace_maes_bscv, _trace_mapes_shallow, _trace_mapes_deep, _trace_mapes_prune, _trace_mapes_bscv, _trace_mses_shallow, _trace_mses_deep, _trace_mses_prune, _trace_mses_bscv, _trace_rmses_shallow, _trace_rmses_deep, _trace_rmses_prune, _trace_rmses_bscv, _trace_r2s_shallow, _trace_r2s_deep, _trace_r2s_prune, _trace_r2s_bscv]
    _layout = go.Layout(title='Comparison of Errors for Decision Tree Variants', yaxis=dict(title='Error'), boxmode='group', xaxis=dict(tickangle=90))
    go.Figure(data=_data, layout=_layout)
    return
